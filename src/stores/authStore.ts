...
import type { Json, ProfileRow, ProfileUpdate } from '@/types/database'
...
export const useAuthStore = defineStore('auth', () => {
  ...
  function subscribeToAuthChanges() {
    ...
    const { data } = supabase.auth.onAuthStateChange((event, nextSession) => {
      ...
      void queueSessionSync(nextSession).catch(async (error: unknown) => {
        ...
        if (nextSession) await supabase.auth.signOut({ scope: 'local' }).catch(() => undefined)
  ...
  async function register(details: RegistrationDetails): Promise<RegistrationResult> {
    ...
    try {
      ...
      clearError()
      return { requiresEmailConfirmation: false, profile: currentUser.value }
    } catch (error) {
      if (sessionCreated) {
        await supabase.auth.signOut({ scope: 'local' }).catch(() => undefined)
        await queueSessionSync(null)
